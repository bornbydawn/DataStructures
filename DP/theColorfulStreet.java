...
class TheColorfulStreet {
    ...
    public static void print() throws IOException {
        ...
        int minCost = Math.min(getMinCost(0, 0, arrSize), Math.min( getMinCost(0, 1, arrSize), getMinCost(0, 2, arrSize)));

        System.out.println(minCost);
        }
    }

    static int getMinCost(int row, int column, int arrSize) {
        ...
        return dp[row][column];
         ...
         return dp[row][column] = costArray[row][column] + Math.min(getMinCost(row + 1, allowedIndex[column][0],arrSize), getMinCost(row + 1, allowedIndex[column][1],arrSize));
